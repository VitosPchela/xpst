...
var xslTransform = {
	...
	init: function(){
		...
		// log the version
		jQuery.log( 'xslTransform:init(): version ' + xslTransform.version );
	},
	...
	serialize: function( data ){
		jQuery.log( 'serialize(): received ' + typeof(data) );
	 ...
	 */
	load: function( xml ){
		jQuery.log( 'load(): received ' + typeof(xml) );
		// the result
		var r;
	...
	loadString: function( str ){
		jQuery.log( 'loadString(): ' + str + '::' + typeof(str) );

		// use Sarissa to generate an XML doc
		var p = new DOMParser();
		var xml = p.parseFromString( str, 'text/xml' );
		if( !xml ){
			jQuery.log( 'loadString(): parseFromString() failed' );
			return false;
		}
		return xml;
	},
	...
	loadFile: function( url ){
		jQuery.log( 'loadFile(): ' + url + '::' + typeof(url) );

		if( !url ){
			jQuery.log( 'ERROR: loadFile() missing url' );
			return false;
		}
		...
		this.xhrsuccess = function(data,str){
			jQuery.log( 'loadFile() completed successfully (' + str + ')' );
			doc = data;
			return true;
		};
		...
		this.xhrerror = function(xhr,err){
			...
			window.DEBUG = true;
			jQuery.log( 'loadFile() failed to load the requested file: (' + err + ') - xml: ' + xhr.responseXML + ' - text: ' + xhr.responseText );
			jQuery.log(xhr.responseXML.documentElement.nodeName);
			doc = null;
			return false;
		};
		...
		if( !doc ){
			jQuery.log( 'ERROR: document ' + url + ' not found (404), or unable to load' );
			return false;
		}
		// check for success but no data
		if( doc.length == 0 ){
			jQuery.log( 'ERROR: document ' + url + ' loaded in loadFile() has no data' );
			return false;
		}
		return doc;
	},
	...
	transform: function( xsl, xml, options ){
		...
		jQuery.log( 'transform(): ' + xsl + '::' + xml + '::' + options.toString() );
		...
		if( options.xpath && xml.doc && !jQuery.browser.msie ){
			...
			$.log( 'transform(): xpath has been run...resulting doc: ' + (this.serialize(xml.doc)) );
		}
		...
		if( options.params && processor ){
			jQuery.log( 'transform(): received xsl params: ' + options.params.toString() );
		...
		jQuery.log( 'transform(): Sarissa parse text: ' + errorTxt );
		if( errorTxt != Sarissa.PARSED_OK ){
			...
			jQuery.log( 'transform(): error in transformation: ' + Sarissa.getParseErrorText(result.doc) );
			return result;
		}
...
jQuery.fn.getTransform = function( xsl, xml, options ){
	...
	jQuery.log( 'getTransform: ' + xsl + '::' + xml + '::' + settings.toString() );

	// must have both xsl and xml
	if( !xsl || !xml ){
		jQuery.log( 'getTransform: missing xsl or xml' );
		return;
	}
	...
	return this.each( function(){
		...
		if( re ){
			...
			jQuery.log( 'getTransform(): found an xml declaration and removed it' );
		}
		...
		if( settings.eval && trans.scripts ){
			if( trans.scripts.length > 0 ){
				jQuery.log( 'Found text/javascript in transformed result' );
