...
if( isObject( MessageBoxHint ) )
   MessageBoxHint.delete();
...
function MBSetText(%text, %frame, %msg)
{
   ...
   %text.setText("<just:center><font:Arial Bold:20>" @ %msg);
...
{
   MessageBoxOKOld(%title, %message, %callback);
...
function MessageBoxOKCancel(%title, %message, %callback, %cancelCallback)
{
   ...
   MessageBoxOKCancelOld(%title, %message, %callback, %cancelCallback);
...
function MessageBoxYesNo(%title, %message, %yesCallback, %noCallback)
{
   ...
   MessageBoxYesNoOld(%title, %message, %yesCallback, %noCallback);
...
function MessageBoxHint(%message,%callback, %prevCallback, %nextCallback)
{

   MessageBoxHintOld(%message, %callback, %prevCallback, %nextCallback);
...
function MessageBoxHintOld(%message,%callback, %prevCallback, %nextCallback)
{
   Canvas.pushDialog(MessageBoxHint);
   MBSetText(MBHintText, MBHintFrame, %message);
   MessageBoxHint.callback = %callback;
   MessageBoxHint.prevCallback = %prevCallback;
   MessageBoxHint.nextCallback = %nextCallback;
   if($hintCount <=1 || ($hintCount > 1 && $hintNumber == 0))
      MessageBoxHintPrev.setActive(false);
   else
      MessageBoxHintPrev.setActive(true);
   if($hintCount <=1 || ($hintCount > 1 && $hintNumber == $hintCount - 1))
      MessageBoxHintNext.setActive(false);
   else
      MessageBoxHintNext.setActive(true);
}

function MessageBoxHint::onSleep( %this )
{
   %this.callback = "";
   %this.prevCallback = "";
   %this.nextCallback = "";
