...
// Data 
//-----------------------------------------------------------------------------
uniform sampler2D diffuseMap, viewMap;
uniform vec3 bloomcutoff, bloomamount;
...
void main()
{
	...
	gl_FragColor.w = max(max(gl_FragColor.x, gl_FragColor.y), gl_FragColor.z);
	gl_FragColor.w *= texture2D(viewMap, TEX0.xy).x * 2.0;
	gl_FragColor.xyz = (gl_FragColor.xyz - bloomcutoff) * bloomamount;
}
